...
{self.console_msg("4")} Share Bot {Fore.RED}{self.status["shares"]}
    ...
    def update_title(self, bot, remaining, rl = False):
        if clear == "cls":
            os.system(clear)
            ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok AIO | Sent: {self.sent} | Cooldown: {remaining}s | Developed by BangsNgek")
            print(ascii_text)
    ...
    def main(self):
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TikTok AIO | Developed by BangsNgek")
